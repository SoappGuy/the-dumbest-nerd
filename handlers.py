...
async def summarise(event):
    ...
    # промпт для джипіті
    prompt = f'''Зараз тобі буде надано повідомлення, твоє завдання - надати список тем 
                 (максимум 10), які обговорювалися в розмові, 
                 (нічого не вигадуй, якщо ти не можеш зрозуміти що обговорювалося 
                 краще взагалі не додавай цю тему), починай з найбільш обговорюваних тем 
                 і закінчуй тими, які були коротко згадані 
                 (не згадуй подібні теми та відповідай українською): \n'''
    prompt += f'{" -- ".join(messages_text[::-1])}'
